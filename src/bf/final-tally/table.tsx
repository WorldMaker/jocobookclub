  ...
  const oldestBlock = finalTally.oldest
    ? (
      <p class='block'>
        ...
        <time datetime={finalTally.oldest.toISOString()}>
          {finalTally.oldest.toLocaleDateString(undefined, {
            dateStyle: 'short',
          })}
        </time>.
        ...
        {(finalTally.supportThreshold ?? 0) > 0
          ? ` ${finalTally.uncounted} ballots were either inactive or did not support at least ${
            finalTally.supportThreshold?.toLocaleString(undefined, {
              style: 'percent',
            })
          } of the ballot.`
          : ''}
  ...
  return (
    <>
      <p class='block'>
        ...
        <time class='has-background-info-dark' datetime={updated.toISOString()}>
