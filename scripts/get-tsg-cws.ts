  ...
  if (test(content)) {
    ...
    if (isBookAttrs(attrs)) {
      ...
      if (Object.keys(cwList).length === 0) {
        ...
        const intentional = Array.from<HTMLElement>(dom.window.document.querySelectorAll('.book-pane > p'))
          .some(p => typeof p.textContent === 'string' && p.textContent.includes("doesn't have any"))
        if (!intentional) {
